...
import org.idtaleb.skyline.entities.*;
...
public interface TaskRepository extends CrudRepository<TaskApp, String> {
    ...
    @Query("SELECT NEW org.idtaleb.skyline.entities.ProgressProject(p.name, " +
    ...
    @Query("SELECT NEW org.idtaleb.skyline.entities.CountTasksInKanbanItem(k.name, COUNT(t)) FROM tasks t, kanban_items k WHERE t.user.id = :userId AND t.kanbanItem.id = k.id GROUP BY k.id,k.name ORDER BY k.KanbanItemOrder")
    List<CountTasksInKanbanItem> countTasksInKanbanItems(@Param("userId") String userId);

    @Query("SELECT NEW org.idtaleb.skyline.entities.CountTasksDate(t.creationDate, COUNT(t)) FROM tasks t WHERE t.user.id = :userId GROUP BY t.creationDate ORDER BY t.creationDate")
